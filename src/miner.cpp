...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("sorsMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("sorsMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("sorsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sors-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("sorsMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running sorsMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadsorsMiner() exception");
    } catch (...) {
        LogPrintf("ThreadsorsMiner() exception");
    }

    LogPrintf("ThreadsorsMiner exiting\n");
}
